...
module.exports = {
	...
	async execute(interaction) {
		...
		await interaction.reply({ content : `Just checking to see if the GamerTag ${gamerTag} exists...`, ephemeral: true });
		try {
			...
			if (lastMatch.id && !await players.findOne({ gamerTag })) {
				...
				await interaction.followUp({ content: `GamerTag exists! I've added ${gamerTag} to the Database to check for new matches.`, ephemeral: true });
			}
		}
		catch (err) {
			console.log(err);
			await interaction.followUp({ content : 'Something went wrong while trying to add the GamerTag. Is the GamerTag correct?', ephemeral: true });
		}
	},
};
